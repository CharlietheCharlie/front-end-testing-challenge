# Frontend Testing Challenge: A Playground for Frontend Testing 🧪
...
# Front-end-testing-challenge: 前端測試修練場 🧪

Frontend Testing Challenge 是一個專為前端開發者設計的互動式測試練習平台。它提供了一個引導式的環境，讓您能夠動手練習 **React Testing Library (RTL)**、**Jest** 以及 **Playwright**。

不同於枯燥的閱讀文檔，Frontend Testing Challenge 讓您直接在瀏覽器中編寫測試程式碼，並針對真實的互動組件進行驗證，獲得即時的反饋。

## ✨ 主要功能

*   **循序漸進的學習路徑**：
    *   **DOM 與無障礙性**：掌握語意化查詢 (`getByRole`, `getByLabelText`)。
    *   **事件處理**：模擬使用者行為 (`userEvent.type`, `click`, `hover`)。
    *   **非同步與模擬**：征服 `fetch`、`useEffect` 以及非同步測試。
    *   **進階 Hooks**：測試 Custom Hooks、計時器 (Timers) 與閉包。
    *   **Playwright**：端對端 (E2E) 測試場景，包含網路攔截 (Mocking) 與 iFrame 處理。
    *   **進階整合**：Context Providers、狀態管理、Portals (跳窗) 與錯誤邊界 (Error Boundaries)。
*   **瀏覽器端執行**：使用基於正則表達式 (Regex) 的靜態分析引擎，無需後端即可即時驗證測試邏輯。
*   **無障礙優先**：內建 "Roles Cheatsheet" (角色速查表)，推廣最佳的無障礙測試實踐。
*   **雙語支援**：隨時切換 英文 / 繁體中文 介面。
...
4.  打開瀏覽器並訪問 `http://localhost:5173`
